...
# ---------------------------------------------------
# Defining creation of new clust with new Marker
# ---------------------------------------------------
def createMarker(id):
    marker = Marker()
    marker.id = id
...
def chatCallback(msg):
    ...
    for idx, (range1, range2) in enumerate(zip(msg.ranges[:-1], msg.ranges[1:])):
        ...
        # Get the last makers points and append it the new calculated point
...
# ---------------------------------------------------
# Creating node with subscriber and publisher
# ---------------------------------------------------
def main():
    rospy.init_node('subscriber_clust', anonymous=True)
    rospy.Subscriber('/left_laser/laserscan', LaserScan, chatCallback)
